...
// Hybrid admin auth that works both locally and on Cloudflare
export async function adminLogin(email: string, password: string) {
  try {
    console.log("Admin login attempt for:", email);

    // Check if we're running on Cloudflare (no cookies available)
    const isCloudflare = typeof window !== "undefined" || !cookies;

    if (isCloudflare) {
      // For Cloudflare Pages Functions, we'll handle this differently
      return {
        error:
          "Admin login not available in static export mode. Use client-side auth.",
      };
    }

    // Skapa Supabase client med cookie-hantering (local development)
...
export async function createAdminAccount(email: string, password: string) {
  try {
    ...
    if (isCloudflare) {
      return {
        error: "Admin account creation not available in static export mode.",
      };
    }
      ...
      if (authError.message.includes("already been registered")) {
        ...
        if (existingUser) {
          ...
          if (!existingProfile) {
            ...
            if (profileError) {
              console.error(
                "Profile creation error for existing user:",
                profileError,
              );
              return {
                error: `Kunde inte skapa profil för befintlig användare: ${profileError.message}`,
              };
            }
            ...
            return {
              success: true,
              message:
                "Profil skapad för befintlig användare! Du kan nu logga in.",
              user: {
                id: existingUser.id,
                email: existingUser.email,
                role: "admin",
            ...
            return {
              error:
                "Användare finns redan och har en profil. Logga in istället.",
            };
          }
        }
      }
...
export async function signOut() {
  try {
    ...
    if (isCloudflare) {
      return { error: "Sign out not available in static export mode." };
    }
