...
export async function getCrowdvineProductByHandle(options: {
  handle: string;
  locale?: AppLocale;
  host?: string | null;
  ...
  if (handle.startsWith("wine-box-")) {
    const wineBoxId = handle.replace("wine-box-", "");
  ...
  const wineEnrichment = {
    ...
    alcohol_percentage:
      i.alcohol_percentage != null && Number.isFinite(Number(i.alcohol_percentage))
        ...
        : null,
    food_pairing:
      extractWineArray(i.food_pairing, localeForWine)?.filter((s: string) =>
        s?.trim(),
      ) ?? null,
    soil_type: extractWineText(i.soil_type, localeForWine),
    elevation_masl:
    ...
    style_scale:
      ...
      Number(i.style_scale) <= 5
        ...
        : null,
    ageing: extractWineText(i.ageing, localeForWine),
    winemaker_notes: extractWineText(i.winemaker_notes, localeForWine),
    awards:
      extractWineArray(i.awards, localeForWine)?.filter((s: string) => s?.trim()) ??
      null,
