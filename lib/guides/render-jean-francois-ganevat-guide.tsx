...
const PAGE_PATH = jeanFrancoisGanevatGuide.path;

export async function buildJeanFrancoisGanevatGuideMetadata(): Promise<Metadata> {
  const config = await getSiteConfig();
  const pageUrl = `${config.baseUrl}${PAGE_PATH}`;
  const title = categoryPageTitle(
    jeanFrancoisGanevatGuide.metaTitle,
    config.siteName,
  );

  return {
    title,
    description: jeanFrancoisGanevatGuide.metaDescription,
    alternates: {
      canonical: pageUrl,
      languages: {
        en: pageUrl,
...
export async function renderJeanFrancoisGanevatGuidePage() {
  const config = await getSiteConfig();
  ...
  const articleJsonLd = {
    ...
    "@type": "Article",
    headline: jeanFrancoisGanevatGuide.h1,
    description: jeanFrancoisGanevatGuide.metaDescription,
    url: pageUrl,
    isPartOf: {
      "@type": "WebSite",
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: jeanFrancoisGanevatGuide.breadcrumbShort,
        item: pageUrl,
      },
    ],
  };
      ...
      <div className="mx-auto max-w-3xl px-6 py-16">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbPage>
                {jeanFrancoisGanevatGuide.breadcrumbShort}
              </BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        <h1 className="mt-6 text-3xl font-bold tracking-tight md:text-4xl">
          {jeanFrancoisGanevatGuide.h1}
        </h1>

        {jeanFrancoisGanevatGuide.sections.map((section) => (
        ...
        <nav className="mt-14 space-y-3 border-t border-border pt-8 text-sm">
          <h2 className="mb-4 text-xl font-semibold text-foreground">
            {jeanFrancoisGanevatGuide.furtherReadingHeading}
          </h2>
          {jeanFrancoisGanevatGuide.links.map((link) => (
            <p key={link.href}>
              <Link
                href={link.href}
