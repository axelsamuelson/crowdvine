...
const PAGE_PATH = joskoGravnerGuide.path;

export async function buildJoskoGravnerGuideMetadata(): Promise<Metadata> {
  const config = await getSiteConfig();
  const pageUrl = `${config.baseUrl}${PAGE_PATH}`;
  const title = categoryPageTitle(
    joskoGravnerGuide.metaTitle,
    config.siteName,
  );

  return {
    title,
    description: joskoGravnerGuide.metaDescription,
    alternates: {
      canonical: pageUrl,
      languages: {
        en: pageUrl,
...
export async function renderJoskoGravnerGuidePage() {
  const config = await getSiteConfig();
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: joskoGravnerGuide.breadcrumbShort,
        item: pageUrl,
      },
    ],
  };
      ...
      <div className="mx-auto max-w-3xl px-6 py-16">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbPage>
                {joskoGravnerGuide.breadcrumbShort}
              </BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        <h1 className="mt-6 text-3xl font-bold tracking-tight md:text-4xl">
          {joskoGravnerGuide.h1}
        </h1>

        {joskoGravnerGuide.sections.map((section) => (
        ...
        <nav className="mt-14 space-y-3 border-t border-border pt-8 text-sm">
          <h2 className="mb-4 text-xl font-semibold text-foreground">
            {joskoGravnerGuide.furtherReadingHeading}
          </h2>
          {joskoGravnerGuide.links.map((link) => (
            <p key={link.href}>
              <Link
                href={link.href}
