...
const PAGE_PATH = radikonGuide.path;

export async function buildRadikonGuideMetadata(): Promise<Metadata> {
  const config = await getSiteConfig();
  const pageUrl = `${config.baseUrl}${PAGE_PATH}`;
  const title = categoryPageTitle(radikonGuide.metaTitle, config.siteName);

  return {
    title,
    description: radikonGuide.metaDescription,
    alternates: {
      canonical: pageUrl,
      languages: {
        en: pageUrl,
...
export async function renderRadikonGuidePage() {
  const config = await getSiteConfig();
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 3,
        name: radikonGuide.breadcrumbShort,
        item: pageUrl,
      },
    ],
  };
      ...
      <div className="mx-auto max-w-3xl px-6 py-16">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage>{radikonGuide.breadcrumbShort}</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        ...
        {radikonGuide.sections.map((section) => (
        ...
        <nav className="mt-14 space-y-3 border-t border-border pt-8 text-sm">
          <h2 className="mb-4 text-xl font-semibold text-foreground">
            {radikonGuide.furtherReadingHeading}
          </h2>
          {radikonGuide.links.map((link) => (
            <p key={link.href}>
              <Link
                href={link.href}
