...
DROP POLICY IF EXISTS p_bookings_access ON bookings;
CREATE POLICY p_bookings_access ON bookings
FOR SELECT USING (
...
DROP POLICY IF EXISTS p_producers_access ON producers;
CREATE POLICY p_producers_access ON producers
FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.access_granted_at IS NOT NULL)
);
...
DROP POLICY IF EXISTS p_pallet_zones_access ON pallet_zones;
CREATE POLICY p_pallet_zones_access ON pallet_zones
FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.access_granted_at IS NOT NULL)
);
...
DROP POLICY IF EXISTS p_order_reservations_access ON order_reservations;
CREATE POLICY p_order_reservations_access ON order_reservations
FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.access_granted_at IS NOT NULL)
);
...
DROP POLICY IF EXISTS p_order_reservation_items_access ON order_reservation_items;
CREATE POLICY p_order_reservation_items_access ON order_reservation_items
FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles p WHERE p.id = auth.uid() AND p.access_granted_at IS NOT NULL)
);
