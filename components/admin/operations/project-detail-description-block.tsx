...
export function ProjectDetailDescriptionBlock({
  projectId,
  initialDescription,
  onSaved,
  variant = "page",
  ...
  async function save() {
    ...
    setSaving(true)
    try {
      const updated = await updateProject(projectId, { description: next })
      setDescription(next ?? "")
      setDraft(next ?? "")
      setEditing(false)
  ...
  const wrap = cn(
    variant === "embedded"
      ...
      : "rounded-2xl border border-gray-200/90 bg-white p-4 shadow-sm dark:border-[#1F1F23] dark:bg-[#0F0F12] dark:shadow-none sm:p-5",
  )

  const iconSize = variant === "embedded" ? "size-8" : "size-9"
    ...
    <div className={cn(wrap, "space-y-3")}>
      <div className="flex min-w-0 items-start gap-2">
        <div className="min-w-0 flex-1 space-y-2">
          <Label
            htmlFor={editing ? `project-desc-${projectId}` : undefined}
            className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-zinc-500"
          >
            {heading}
          </Label>
          {!editing && (
        ...
        {!editing && (
          <Button
            ...
            aria-label="Redigera beskrivning"
            onClick={() => {
              setDraft(description)
              setEditing(true)
            }}
      ...
      {editing && (
        <div className="flex min-w-0 items-start gap-2">
          <Textarea
            ...
            className={cn(
              "min-w-0 flex-1 resize-y py-2 text-sm min-h-[120px]",
      ...
      {editing && (
        <p className="text-[11px] text-gray-500 dark:text-gray-500">
          Spara med bocken eller ⌘/Ctrl+Enter. Tom + spara tar bort beskrivningen.
        </p>
      )}
    </div>
  )
