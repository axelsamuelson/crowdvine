...
export function LatamMap({ departmentsWithEvents = [], countriesWithEvents = [] }: LatamMapProps) {
  ...
  const [peruDeptData, setPeruDeptData] = useState<any>(null)
  const [latamDotsData, setLatamDotsData] = useState<LatamCountryDots[]>([])
  const [peruDotsData, setPeruDotsData] = useState<PeruDot[]>([])
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    async function loadData() {
      try {
        const [world, peruDept, latamDots, peruDots] = await Promise.all([
          fetch("/countries-110m.json").then((res) => res.json()),
          fetch("/peru_departamental_simple.json").then((res) => res.json()),
          fetch("/latam-dots.json").then((res) => res.json()),
          fetch("/peru-dots.json").then((res) => res.json()),
        ])

        setWorldData(world)
        setPeruDeptData(peruDept)
        setLatamDotsData(latamDots)
        setPeruDotsData(peruDots)
        setIsLoading(false)
      } catch (error) {
  ...
  const peruDepartments = useMemo(() => {
    if (!peruDeptData) return []
    return feature(peruDeptData as any, (peruDeptData as any).objects.peru_departamental_simple)
        ...
        <AnimatePresence mode="wait">
          {!isZoomedIntoPeru ? (
            ...
            >
              ...
              {limaProjectedCoords && <EventLocationDot x={limaProjectedCoords.x} y={limaProjectedCoords.y} />}
            </motion.g>
          ) : (
            <motion.g
              key="peru-view"
            ...
            >
              {peruDepartments.map((dept, deptIndex) => {
                ...
                return (
                  <path
                    ...
                    onClick={() => {
                      if (hasEvent) {
                        const eventSlug = normalizedName.toLowerCase().replace(/\s+/g, "-")
                        window.open(`https://hack0.dev/events/${eventSlug}`, "_blank", "noopener,noreferrer")
                      }
                    }}
                  />
                )
              ...
              {departmentEventCoords.map((coord, index) => (
                <EventLocationDot key={coord.department} x={coord.x} y={coord.y} delay={index * 0.15} />
              ))}
            </motion.g>
          )}
        </AnimatePresence>
