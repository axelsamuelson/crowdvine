-- Site dimension + intent sessions.
-- Supersedes the clean-view definition from 175–176 and adds analytics_intent_sessions.

-- A. Single source of truth for site from page_url
CREATE OR REPLACE FUNCTION analytics_site(page_url text)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
AS $$
  SELECT CASE
    lower(
      regexp_replace(
        COALESCE(
          substring(nullif(btrim(page_url), '') FROM '://([^/?#:]+)'),
          ''
        ),
        '^www\.',
        ''
    ...
    WHEN 'pactwines.com' THEN 'pact'
    WHEN 'dirtywine.se' THEN 'dirtywine'
    ELSE NULL
  END;
$$;

COMMENT ON FUNCTION analytics_site(text) IS
  'Derive analytics site from page_url: pact | dirtywine | NULL (localhost, previews, empty, other).';

-- Recreate views from scratch: CREATE OR REPLACE cannot reorder/rename columns
-- (e.g. inserting site before user_id on analytics_intent_sessions).
DROP VIEW IF EXISTS analytics_intent_sessions;
DROP VIEW IF EXISTS analytics_sessions_clean;

-- B. Clean sessions with session-level site; exclude unknown/null sites
...
COMMENT ON VIEW analytics_sessions_clean IS
  'user_events excluding internal profiles, bots, server_ sessions, localhost referrers, internal-tagged sessions, and sessions with no pact/dirtywine page_url. Includes session-level site column.';

-- C. Intent sessions (carry site from clean events)
