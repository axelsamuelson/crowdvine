...
export async function POST(request: Request) {
  try {
    ...
    if (existingItem) {
      ...
      console.log("🛒 [ADD-QUANTITY] Item exists, incrementing from", existingItem.quantity, "to", newQuantity);
      ...
      if (updateError) {
        console.error("🛒 [ADD-QUANTITY] Error updating quantity:", updateError);
        return NextResponse.json({ error: "Failed to update quantity" }, { status: 500 });
      }
    } else {
      // Item doesn't exist - insert with requested quantity
      console.log("🛒 [ADD-QUANTITY] Item not in cart, inserting with quantity", quantity);
