  ...
  try {
    ...
    if (zonesError) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch zones", details: zonesError },
        { status: 500 },
      );
    }
    ...
    if (pickupError) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch pickup zones", details: pickupError },
        { status: 500 },
      );
    }
    ...
    if ("error" in geocodeResult) {
      ...
      console.log(
        `✅ Geocoded successfully: ${geocodeResult.lat}, ${geocodeResult.lon}`,
      );
      geocodingInfo = {
        address: testAddress,
