  ...
  try {
        ...
        if (cartItems && cartItems.length > 0) {
          ...
          if (winesError) {
            ...
            if (wines && wines.length > 0) {
              const producerIds = [
                ...new Set(wines.map((wine) => wine.producer_id)),
              ];
              ...
              if (producersError) {
                ...
                debugResults.results.push({
                  ...
                  pickupZones:
                    producers?.map((p) => p.pallet_zones).filter(Boolean) || [],
                });
                console.log(
                  `✅ Found ${producers?.length || 0} producers with pickup zones`,
                );
              }
            }
          }
    ...
    try {
      const { data: deliveryZones, error: deliveryZonesError } = await adminSb
        .from("pallet_zones")
        .select(
          "id, name, center_lat, center_lon, radius_km, country_code, zone_type",
        )
      ...
      if (deliveryZonesError) {
        ...
        if ("error" in geocodeResult) {
          ...
          console.log(
            `✅ Geocoding successful: ${geocodeResult.lat}, ${geocodeResult.lon}`,
          );
          ...
          for (const zone of deliveryZones || []) {
            if (zone.center_lat && zone.center_lon && zone.radius_km) {
              ...
              console.log(
                `  ${matches ? "✅" : "❌"} ${zone.name}: ${distance.toFixed(2)}km (radius: ${zone.radius_km}km) ${matches ? "MATCHES" : "OUT OF RANGE"}`,
              );
