...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: rewardResult, error } = await supabase.rpc('create_invitation_reward_discount', {
      p_user_id: userId,
    ...
    if (fetchError) {
      ...
      return NextResponse.json({ error: "Failed to fetch discount code details" }, { status: 500 });
    }
