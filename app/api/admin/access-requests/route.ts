...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (simpleError) {
      ...
      console.error(
        "DEBUG: Simple update error details:",
    ...
    if (fullError) {
      ...
      console.error(
        "DEBUG: Full update error details:",
    ...
    if (status === "approved") {
      ...
      const { data: authUser, error: userError } =
        await supabase.auth.admin.getUserByEmail(accessRequest.email);

      if (userError || !authUser.user) {
        console.log(
          "DEBUG: User not found in auth, will grant access when they sign up:",
          accessRequest.email,
        );
        ...
        try {
          ...
          if (signupResponse.ok) {
            ...
            console.log(
              "DEBUG: Signup URL generated for:",
              accessRequest.email,
              "URL:",
              signupUrl,
            );
            ...
            try {
              console.log(
                "DEBUG: Sending approval email to:",
                accessRequest.email,
                "with URL:",
                signupUrl,
              );
              ...
              if (emailSent) {
                console.log(
                  "DEBUG: Approval email sent successfully to:",
                  accessRequest.email,
                );
              } else {
                console.log(
              ...
              console.log(
                "DEBUG: Email sending failed, but signup URL generated:",
                signupUrl,
              );
            ...
            console.error(
              "DEBUG: Failed to generate signup URL for:",
              accessRequest.email,
            );
          }
        } catch (signupError) {
        ...
        console.log(
          "DEBUG: User exists, granting access immediately:",
          accessRequest.email,
        );
  ...
  try {
    ...
    try {
      ...
      if (tokensError) {
        ...
        console.log(
          `   ✅ Deleted ${deletedTokens?.length || 0} access tokens`,
        );
      }
    ...
    try {
      ...
      if (invitationsError) {
        ...
        console.log(
          `   ✅ Deleted ${deletedInvitations?.length || 0} invitation codes`,
        );
      }
    } catch (invitationsDeleteError) {
      console.error(
        "Exception deleting invitation codes:",
