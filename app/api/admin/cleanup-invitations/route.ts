...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📊 Found ${orphanedInvitations?.length || 0} invitations with used_by references`);
    ...
    if (orphanedInvitations) {
      for (const invitation of orphanedInvitations) {
        ...
        if (!invitation.profiles) {
          ...
          console.log(`🗑️ Orphaned invitation found: ${invitation.code} (used_by: ${invitation.used_by})`);
        }
      }
    }
    
    console.log(`🔍 Found ${orphanedInvitationIds.length} orphaned invitations to clean up`);
    ...
    console.log(`✅ Successfully cleaned up ${updatedInvitations?.length || 0} orphaned invitations`);
