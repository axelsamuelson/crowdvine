...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Cleaned up ${tokensCleaned} expired access tokens and ${invitationsCleaned} expired invitation codes`);
...
export async function GET() {
  try {
    ...
    const { count: expiredInvitationsCount, error: invitationsError } = await supabase
      .from('invitation_codes')
      .select('*', { count: 'exact', head: true })
      .lt('expires_at', new Date().toISOString())
      .eq('is_active', true);
    ...
    if (invitationsError) {
      console.error('Error counting expired invitation codes:', invitationsError);
    }
