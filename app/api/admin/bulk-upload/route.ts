...
async function findSimilarProducers(
  supabase: any,
  producerName: string,
): Promise<
  Array<{
...
> {
  ...
  const { data: producers } = await supabase.from("producers").select("name");
...
async function parseCSV(csvContent: string): Promise<{
  ...
  if (missingHeaders.length > 0) {
    ...
    errors.push(
      `Make sure to use the latest CSV template downloaded from the bulk upload page.`,
    );
    return { products: [], errors };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      if (!product.wine_name)
        rowErrors.push(`Row ${i + 1}: Wine name is required`);
      ...
      if (product.cost_amount <= 0)
        rowErrors.push(`Row ${i + 1}: Cost must be greater than 0`);
      if (!product.producer_name)
        rowErrors.push(`Row ${i + 1}: Producer name is required`);
  ...
  for (let index = 0; index < products.length; index++) {
    ...
    try {
      ...
      if (existingProducer) {
        ...
        const similarProducers = await findSimilarProducers(
          supabase,
          product.producer_name,
        );

        if (similarProducers.length > 0) {
          const suggestions = similarProducers.map(
            (p) => `${p.name} (${Math.round(p.similarity * 100)}% match)`,
          );
          ...
          if (producerError) {
            errors.push(
              `Producer "${product.producer_name}": ${producerError.message}`,
            );
            continue;
          }
      ...
      // Create wine
      const { error: wineError } = await supabase.from("wines").insert({
        id: randomUUID(),
