...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate signup URL
    const signupResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/generate-signup-url`, {
      method: "POST",
    ...
    if (emailSent) {
      ...
      console.error("❌ HOTMAIL OPTIMIZED: Failed to send approval email to:", email);
      return NextResponse.json(
    ...
    console.error("❌ HOTMAIL OPTIMIZED: Error in send-hotmail-optimized-email:", error);
    return NextResponse.json(
