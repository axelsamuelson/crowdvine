  ...
  try {
    ...
    const healthReport = {
      ...
      summary: {
        ...
        totalWinesWithLabelImage: wines?.filter(w => w.label_image_path)?.length || 0,
    ...
    const winesWithoutImages = wines?.filter(wine => 
    ...
    const totalImages = healthReport.summary.totalWineImages + healthReport.summary.totalWinesWithLabelImage;
    const healthyImages = healthReport.wineImages.filter(img => img.status === 'healthy').length + 
                         healthReport.wines.filter(wine => wine.status === 'healthy').length;
    
    healthReport.summary.healthScore = totalImages > 0 ? Math.round((healthyImages / totalImages) * 100) : 100;
