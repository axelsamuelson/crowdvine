...
export async function GET(_: Request, { params }: { params: { handle: string } }) {
  ...
  const { data: i, error } = await sb
    .from('campaign_items')
    .select('id, handle, wine_name, vintage, label_image_path, grape_varieties, color, price_t100_cents, price_t200_cents, price_t300_cents, price_t400_cents, price_t500_cents, price_t600_cents, price_t700_cents')
