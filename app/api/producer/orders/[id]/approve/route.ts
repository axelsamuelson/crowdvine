...
export async function POST(
  ...
  try {
    ...
    // Only transition from pending_producer_approval -> approved (idempotent)
    if (reservation.status !== "approved") {
      const allowed = ["pending_producer_approval", "placed"];
      if (!allowed.includes(String(reservation.status))) {
        return NextResponse.json(
          { error: `Cannot approve order in status '${reservation.status}'` },
          { status: 400 },
        );
      }
    }
    ...
    const { data: updated, error: updateError } = await sb
      ...
      .update({
        status: "approved",
        producer_approved_at: now,
        producer_approved_by: user.id,
        producer_rejected_at: null,
        producer_rejected_by: null,
      })
      .eq("id", id)
      .select("id, status, producer_approved_at")
      .single();
