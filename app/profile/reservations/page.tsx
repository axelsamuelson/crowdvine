...
export default function ReservationsPage() {
  ...
  const estimatePalletCompletion = (palletReservations: Reservation[]) => {
    const totalItems = palletReservations.reduce(
      (sum, res) =>
        sum + res.items.reduce((itemSum, item) => itemSum + item.quantity, 0),
      0,
    );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Track your wine orders and deliveries
            </p>
          </div>
        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {palletReservations.length} reservation
                            {palletReservations.length !== 1 ? "s" : ""}
                          </p>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-medium text-gray-900">
                            {completion.totalItems} /{" "}
                            {completion.palletCapacity} bottles
                          </div>
                    ...
                    <CardContent className="p-6">
                      <div className="space-y-4">
                        {palletReservations.map((reservation) => {
                          const journeySteps = getOrderJourneySteps(
                            reservation.status,
                          );
