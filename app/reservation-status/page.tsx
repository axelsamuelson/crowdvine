...
export default function ReservationStatusPage() {
  ...
  useEffect(() => {
    ...
    if (!email || (!reservationId && !trackingCode)) {
      setError('Email och antingen reservations-ID eller tracking-kod krävs för att kolla status');
      setLoading(false);
      return;
    }
    ...
    const fetchReservationStatus = async () => {
      try {
        ...
        const response = await fetch(`/api/reservation-status?${params.toString()}`);
        
        if (!response.ok) {
          if (response.status === 404) {
            setError('Reservationen hittades inte. Kontrollera att email och reservations-ID/tracking-kod är korrekt.');
          } else {
    ...
    return (
      <div className="max-w-3xl mx-auto p-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-semibold mb-4">Kunde inte hitta reservationen</h1>
  ...
  if (!reservation) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        <div className="text-center">
          <h1 className="text-2xl font-semibold mb-4">Reservationen hittades inte</h1>
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Reservationsstatus</h1>
        <p className="text-lg text-gray-600">
          Status för din vinreservation
        </p>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p>{new Date(reservation.created_at).toLocaleDateString('sv-SE')}</p>
          </div>
          <div>
        ...
        <div className="space-y-3">
          {reservation.items.map((item, index) => (
            <div key={index} className="flex justify-between items-center p-3 bg-white rounded border">
              <div>
                <p className="font-medium">{item.wine_name} {item.vintage}</p>
                <p className="text-sm text-gray-600">Antal: {item.quantity} st</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600">Prisband: {item.price_band}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-gray-50 rounded-lg p-6 mb-8">
        ...
        <div className="space-y-2">
          ...
          <p>{reservation.address.postcode} {reservation.address.city}</p>
        ...
        <div className="space-y-4">
          {reservation.status === 'placed' && (
            <div className="flex items-start">
              ...
              <div>
                <h3 className="font-medium text-gray-900">Reservation placerad</h3>
                <p className="text-sm text-gray-600">
                  Din reservation har mottagits och väntar på att matchas med andra kunder i samma zon.
                </p>
              </div>
            </div>
          )}
          {reservation.status === 'allocated' && (
            <>
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="font-medium text-gray-900">Allokerad till pall</h3>
                  <p className="text-sm text-gray-600">
                  Din reservation har matchats med andra kunder och allokerats till en pall.
                  </p>
                </div>
              </div>
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="font-medium text-gray-900">Betalning kommer snart</h3>
                  <p className="text-sm text-gray-600">
                    Vi kommer att debitera din sparade betalningsmetod när pallen är redo att skickas.
                  </p>
                </div>
              </div>
            </>
