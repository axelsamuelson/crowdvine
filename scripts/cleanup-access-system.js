  ...
  try {
    ...
    if (profilesWithAccess) {
      ...
      if (deleteError) {
        ...
        console.log(`   ✅ Cleaned up access requests for ${emailsWithAccess.length} users with access`);
      }
    }
    ...
    console.log(`   ✅ Cleaned up ${oldRequests?.length || 0} old pending access requests`);
    ...
    if (profilesWithAccess) {
      ...
      if (tokenDeleteError) {
        ...
        console.log(`   ✅ Cleaned up access tokens for ${emailsWithAccess.length} users with access`);
      }
    }
    ...
    console.log(`   ✅ Cleaned up ${expiredTokens?.length || 0} expired tokens`);
