-- Country code on analytics events (ISO 3166-1 alpha-2).
-- From Vercel x-vercel-ip-country via session cookie pact_geo_country.
-- Do NOT store IP addresses — country code only.
--
-- CREATE OR REPLACE cannot insert/reorder view columns (Postgres 42P16).
-- Drop dependent views, then recreate so country_code from user_events
-- flows through base (ue.*) and site stays last.

ALTER TABLE user_events
  ADD COLUMN IF NOT EXISTS country_code text;

COMMENT ON COLUMN user_events.country_code IS
  'ISO 3166-1 alpha-2 from Vercel x-vercel-ip-country (pact_geo_country cookie). Nullable; never store IP.';

CREATE INDEX IF NOT EXISTS idx_user_events_country_code
  ON user_events (country_code);
...
COMMENT ON VIEW analytics_sessions_clean IS
  'user_events excluding internal profiles, bots, server_ sessions, localhost referrers, internal-tagged sessions, and sessions with no pact/dirtywine page_url. Includes visitor_id, country_code, and session-level site.';
