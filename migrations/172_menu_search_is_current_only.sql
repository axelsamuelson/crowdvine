-- Migration 172: Wine search reads only current (latest) menu snapshot per venue.
...
COMMENT ON FUNCTION search_menu_wines IS 'Wine search/browse; only is_current completed menu snapshots per venue.';
COMMENT ON FUNCTION count_search_menu_wines IS 'Row count for search_menu_wines (same filters).';
COMMENT ON FUNCTION search_menu_wines_grouped IS 'Grouped wine search; only is_current completed menu snapshots per venue.';
